...
import hmac
import base64
#App Engine doesn't support requests module
#import requests
import json
...
class GraphAPI(object):
    ...
    def request(
        ...
        try:
            req = urllib2.Request("https://graph.facebook.com/" + path + "?" + urllib.urlencode(args),
                                        data=post_args)
        ...
        if 'json' in headers['content-type'] or 'javascript' in headers['content-type']:
            ...
            result = {"data": response.content,
                      "mime-type": mimetype,
                      "url": response.url}      
        elif "access_token" in parse_qs(content):
